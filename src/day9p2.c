...
ssize_t calc_diff(ssize_t seq[], size_t seq_len) {
    ssize_t new_seq[MAX_SEQ_SIZE];

    bool all_zeroes = true;
    for (int i = 1; i < seq_len; i++) {
        ssize_t v = seq[i] - seq[i - 1];
        if (v != 0) {
            all_zeroes = false;
        }
        new_seq[i - 1] = v;
    ...
    return -calc_diff(new_seq, seq_len - 1) + seq[0];
}

ssize_t calc_line(char *line) {
    ssize_t numbers[MAX_SEQ_SIZE];
    ...
    while (1) {
        ...
        numbers[numbers_count++] = atol(n_str);
    }
